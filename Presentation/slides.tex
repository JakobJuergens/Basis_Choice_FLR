...
\begin{document}
	...
	\begin{frame}{Introduction}
		...
		\begin{itemize}
			\item Continuous underlying process where $x_i(t)$ exists $\forall t \in [T_1, T_2]$ but is only observed at $x_{i}(t_{j,i})$
	...
	%\begin{frame}{Random Function}
	%	The realization of random function can be described as
	%	$${\{X(t,\omega) : t \in \mathbb{E}, \omega \in \Omega\},}$$
	%	where $\mathbb{E}$ is the index set and $X(t, \cdot)$ is
	%	$\mathcal{F}$-measurable function on the sample space $\Omega$.\\
	%	\begin{itemize}
	%		\item It can be shortened to $X(t)$ by omitting $\omega$ or I can
	%		be written as $X(t)(\omega)$.
	%		\item The function is realized when the $X(t)$ exists $\forall t
	%		\in \mathbb{E}$.
	%	\end{itemize}
	%\end{frame}
	...
	\begin{frame}{Basis Expansion}
		...
		$$x_{i}(t) = \sum_{k=1}^{\infty} c_{ik}\phi_{k}(t) \approx
		\sum_{k=1}^{K} c_{ik}\phi_{k}(t), \quad i = 1, \dots, n, \quad \forall t \in
		\mathbb{E}$$
		
		where $\phi_{k}(t)$ is the $k^{th}$ basis function of the expansion
		and $c_{ik}$ is the corresponding coefficient. We truncate the basis at $K$
		to:
		\vspace{0.2cm}
	...
	\begin{frame}{Basis Functions}
		\textbf{B-spline Basis Functions} are polynomial functions defined by
	...
	\begin{frame}{Trade Off between Bias and Variance}
		...
		\begin{itemize}
			\item The larger $K$, the better the fit to the data, but also more fitting noise
	...
	\begin{frame}{Simulation Setup cont.}
		...
		\begin{itemize}
    		\item Four combinations with different number of cubic basis-function $n_{basis} \in \{4,5, \dots ,25\}$ and fourier functions $\{1,3, \dots, 25\}$ to perform regression using basis expansion and the FPCR approach.
