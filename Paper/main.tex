...
\end{figure}
	...
	\end{equation}
	where $\mu(t)$ is the mean function of $X(t)$ and coefficients $\xi_{j}$ are given by $\xi_{j}(\omega) = \int_{0}^{1} X(t)(\omega) \nu_{j}(t)dt$. These coefficients satisfy the following conditions.
	\begin{enumerate}
		\item $\mathbb{E}[\xi_{j}] = 0$
		\item $\mathbb{E}[\xi_{j} \xi_{k}] = \lambda_{j}$, if $j = k$, otherwise 0
		\item Var[$\xi_{j}$] = $\lambda_{j}$
	...
	\begin{proof}
		We know that $K$ has a complete set of eigenvectors ${\nu_{j}}$ in $\mathbb{L}^{2}[0,1]$ and non-negative eigenvalues $\lambda_{j}$. With the solution that $\xi_{j}(\omega) = \int_{0}^{1} X(\omega)(t) \nu_{j}(t)dt$ satisfy the three conclusions by Lemma \ref{Proof1}, We prove this expansion by considering
		\begin{equation}
