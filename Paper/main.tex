...
\begin{document}
	...
	{\color{orange} For the last equation, shouldn't it be approx including following equations regarding truncation error term? or delta times delta is so small that we can neglect(maybe not true)..?} In practice, a typical choice is to use the same basis $\left(\phi_j(t)\right)_{j \in \mathcal{I}}$ and the same truncation parameter $L$ for the basis expansion of the demeaned observations $\left(x_i(t) - \hat{\mu}(t)\right)$ and the functional principal components $\hat{\nu}^m$. This leads to the following simplification of Equation \ref{FPCA_basis_expansion}.
	...
	\section{Application}\label{Application}
		The application uses the methods and insights from the previous sections to predict the octane ratings of the introduced gasoline data set.
