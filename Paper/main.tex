...
\begin{document}
	\title{{\LARGE Scalar-on-Function Regression \\ with Applications to Near-Infrared Spectroscopy}}
...
The polynomial of the form 
	...
	\subsection{Regression using Functional Principal Components}
	
	
	\begin{equation}\label{FPCA_basis_expansion}
		\begin{split}
			\hat{\xi}_{j, i} & = \int_{0}^{1} \left({\color{red} f_i(s) - \hat{\mu}(s)}\right) {\color{blue}\hat{\nu}(s)} \mathrm{d}s
			= \int_{0}^{1} \left({\color{red}\sum_{j \in \mathcal{I}} a_{i,j} \phi_j(s)}\right) \left({\color{blue}\sum_{k \in \mathcal{L}} b_{k} \psi_{k}(s)}\right) \mathrm{d}s \\
			& = \sum_{j \in \mathcal{I}} \left[a_{i,j} \sum_{k \in \mathcal{L}} {\color{teal} b_{k} \int_{0}^{1} \phi_j(s) \psi_{k}(s)\mathrm{d}s}\right]
			= \sum_{j \in \mathcal{I}} \left[a_{i,j} \sum_{k \in \mathcal{L}} {\color{teal} Z_{j,k}} \right]\\
			&\approx \sum_{j = 1}^{J} \left[a_{i,j} {\color{violet}\sum_{k = 1}^{K} Z_{j,k}}  \right]
			= \sum_{j = 1}^{J} a_{i,j} {\color{violet}\tilde{Z}_{j}}
		\end{split}
	\end{equation}
	...
	\begin{equation}
		\begin{split}
			Z_{j, k} & = d_k(\omega) \int_{0}^{1} \phi_j(s) \phi_k(s) \mathrm{d}s \quad j = 1, \dots, L \quad k = 1, \dots, L \\
			\tilde{Z}_j& = \sum_{k = 1}^{L} \left[b_k \int_{0}^{1} \phi_j(s) \phi_k(s) \mathrm{d}s \right] \quad j = 1, \dots, L
		\end{split}
	\end{equation}
