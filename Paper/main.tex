...
The focus of this paper is to introduce FLR in a scalar-on-function setting. We will be using the standard FLR framework, which relates functional predictors to a scalar response as follows:
	...
	Consider the case of a data set containing observations $X_i(t)$ of a random function $X(\omega)(t)$, measured at discrete points $t_{j,i}$: 
	
	\begin{equation}
		x_{i}(t_{j,i}) \in \mathbb{R}, \quad i = 1,\: \dots\: ,N, \; j = 1, \: \dots \:, J_i, \; t_{j,i} \in [T_1, T_2]
	\end{equation}
	
 	Each curve $x_i(t)$ exists $\forall t \in [T_1, T_2]$, but is only observed at discrete measurement points. Therefore, this can be seen as set of discretized approximations of the realizations of a random function.
	As in the finite dimensional setting, the concept of identically distributed and independent data is important. This concept generalizes to the case of functional data using the concepts for general random variables.
	 
	An example of such a data set could be the data set of 60 NIR-spectra of gasoline samples. Together with a set of responses, this then can be used to perform functional linear regression, as will be subject of the simulation and application.	 
