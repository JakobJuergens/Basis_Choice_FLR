...
The polynomial of the form 
	...
	\begin{equation}
		\begin{split}
			...
			& \approx \tilde{\alpha} + \sum_{m  = 1}^{M} \left[ \tilde{\xi}^{m,L}_{i}\sum_{j = 1}^{L} \left[ b_j \sum_{k = 1}^{L} n_{k}^m \int_{0}^{1} \phi_{k}(s) \phi_j(s) \mathrm{d}s \right] \right]+ \epsilon(\omega) \\
		\end{split}
	\end{equation}
