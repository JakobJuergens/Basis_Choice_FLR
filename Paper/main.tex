...
The polynomial of the form 
	...
	{\color{red} We have to clean this up theoretically. Having both the mean function and the $\alpha$ in there makes this viable for the model but not for estimation as are estimated in the constant.}
	...
	\begin{equation}
		\begin{split}
			...
			&= \alpha + \int_{0}^{1} \left[ \sum_{j \in \mathcal{I}} b_j \phi_j(s) \mu(s) + \sum_{j \in \mathcal{I}} \left[b_j \sum_{m \in \mathcal{M}} \xi^m(\omega) \nu^m(s) \phi_j(s)\right] \right] \mathrm{d}s + \epsilon(\omega) \\
			&= \alpha + \sum_{j \in \mathcal{I}} b_j \int_{0}^{1} \phi_j(s) \mu(s) \mathrm{d}s + \sum_{j \in \mathcal{I}} \left[b_j \sum_{m \in \mathcal{M}} \xi^m(\omega) \int_{0}^{1}\nu^m(s) \phi_j(s) \mathrm{d}s \right] + \epsilon(\omega) \\
			& \approx \alpha + \sum_{j = 1}^{J} b_j \int_{0}^{1} \phi_j(s) \mu(s) \mathrm{d}s + \sum_{j = 1}^{J} \left[b_j \sum_{m  = 1}^{M} \xi^m(\omega) \int_{0}^{1}\nu^m(s) \phi_j(s) \mathrm{d}s \right] + \epsilon(\omega)
		\end{split}
	\end{equation}
	...
	\begin{equation}
			Y(\omega) \approx \alpha + \sum_{j = 1}^{J} b_j \int_{0}^{1} \phi_j(s) \hat{\mu}(s) \mathrm{d}s + \sum_{j = 1}^{J} \left[b_j \sum_{m  = 1}^{M} \hat{\xi}^m(\omega) \int_{0}^{1} \hat{\nu}^m(s) \phi_j(s) \mathrm{d}s \right] + \epsilon(\omega)
	\end{equation}
	...
	\begin{equation}
		y_i \approx \alpha + \sum_{j = 1}^{L} b_j \int_{0}^{1} \phi_j(s) \hat{\mu}(s) \mathrm{d}s + \sum_{j = 1}^{L} \left[b_j \sum_{m  = 1}^{M}  \tilde{\xi}^{m,L}_{i} \int_{0}^{1} \hat{\nu}^m(s) \phi_j(s) \mathrm{d}s \right] + \epsilon(\omega)
	\end{equation}
