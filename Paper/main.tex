...
\end{itemize}
	...
	\end{enumerate}
	\textit{Proof}. To obtain the first result, we can show that
	\begin{equation}
		\begin{split}
	...
	\begin{equation}
		F(t) = \sum_{i=1}^{\infty} \xi_{i} \nu_{i}(t),
	\end{equation}
	where coefficients $\xi_{i}$ are given by $\xi_{i}(\omega) = \int_{0}^{1} F(t)(\omega) \nu_{i}(t)dt$ and satisfy the following conditions.
	\begin{enumerate}
		\item $\mathbb{E}[\xi_{j}] = 0$
		\item $\mathbb{E}[\xi_{j} \xi_{k}] = \lambda_{j}$, if $j = k$, otherwise 0
		\item Var[$\xi_{j}$] = $\lambda_{j}$
