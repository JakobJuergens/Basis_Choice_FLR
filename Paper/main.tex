...
This is the basis that is needed in order to build polynomials, one of the best known ways of basis expansion.
For the polynomial of the form 

	\begin{equation}
		f(t) = \sum_{i =1}^{\mathcal{I}} c_k(t -w)^{i-1}
	\end{equation}
	$(t -w)$ is the monomial basis function of the sequence
	
	\begin{equation}
		S_{i}^{MB}(t) = (t-w)^i, i = 0, 1, ..., \mathcal{I} 
	\end{equation}
	
	where the shift parameter $w$ usually is specified to be the center of the interval subject to approximation.
   ...
   If we approximate the scalar-on-functional regression by assuming a set of discrete observation points for all realizations of the data generating process as
    ...
    Both functions were created such that the inner product $\langle NIR_{sim}, f \rangle, f \in \{f_1(t), f_2(t)\}$ creates responses similar to the original octane ratings of the gasoline dataset. 
		...
		\end{figure}
		 ...
		 Two different error-terms $\epsilon$ were created by first generating an $i.i.d.$ standard normal error term and then multiplying it by two error variations $\sigma_e $. The error variations represent different signal-to-noise ratios and are created such that the squared multiple correlation coefficient $R^2 = var(Xf) / (var(Xf) + \sigma^2_{e})$ is equal to 0.9 and 0.6. The two error-terms are then used to generate two sets of responses for $f \in \{f_1(t), f_2(t)\}$.
