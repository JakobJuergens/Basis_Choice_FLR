...
The polynomial of the form 
	...
	\begin{equation}\label{basis_exp_transf}
		\begin{split}
			...
			& = \alpha + \sum_{j \in \mathcal{I}} \left[\sum_{k \in \mathcal{L}} c_j d_k(\omega) \textcolor{teal}{ \int_{0}^{1} \phi_j(s) \psi_k(s) \mathrm{d}s}\right] + \epsilon(\omega)
			= \alpha + \sum_{j \in \mathcal{I}} \sum_{k \in \mathcal{L}} c_j d_k(\omega)  \textcolor{teal}{Z_{j, k}} + \epsilon(\omega) \\
			& \approx \alpha + \sum_{j = 1}^{J} \sum_{k = 1}^{K} c_j d_k(\omega) \textcolor{violet}{Z_{j, k}}  + \epsilon(\omega)	
		\end{split}
	\end{equation}
