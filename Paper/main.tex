...
The polynomial of the form 
	...
	\begin{equation}\label{FPCA_basis_expansion}
		\begin{split}
			...
			& = \sum_{j \in \mathcal{I}}\sum_{k \in \mathcal{L}} {\color{teal} a_{i,j} b_{k} \int_{0}^{1} \phi_j(s) \psi_{k}(s)\mathrm{d}s}
			= \sum_{j \in \mathcal{I}} \sum_{k \in \mathcal{L}} {\color{teal} Z_{i,j,k}} 
			\approx \sum_{j = 1}^{J} {\color{violet}\sum_{k = 1}^{K} Z_{i,j,k}} 
			= \sum_{j = 1}^{J} {\color{violet}\tilde{Z}_{i,j}}
		\end{split}
	\end{equation}
	...
	\begin{equation}
		\begin{split}
			Z_{i, j, k} & =  a_{i,j} b_k \int_{0}^{1} \phi_j(s) \phi_k(s) \mathrm{d}s \quad j = 1, \dots, L \quad k = 1, \dots, L \\
			\tilde{Z}_{i, j}& = a_{i,j} \sum_{k = 1}^{L} \left[ b_k \int_{0}^{1} \phi_j(s) \phi_k(s) \mathrm{d}s \right] \quad j = 1, \dots, L
		\end{split}
	\end{equation}
