	...
	\begin{proof}
		...
		\begin{equation}\label{Lemma2}
			\begin{split}
				...
				= & \int_{0}^{1} {\color{red}\int_{0}^{1} \mathbb{E}[{F(s)F(t)}] \nu_{j}(s)} \nu_{k}(t) {\color{red}ds} dt\\
				= & \int_{0}^{1} {\color{red}\left(\int_{0}^{1}c(s,t)\nu_{j}(s)ds \right)} \nu_{k}(t) dt \\
				= & \int_{0}^{1}{\color{red}[K\nu_{j}](t)}\nu_{k}(t)dt\\
				= & \langle K \nu_{j}, \nu_{k} \rangle\\
				= & \langle \lambda_{j} \nu_{j}, \nu_{k} \rangle = \delta_{j,k}\lambda_{j}
			\end{split}
		\end{equation}
		where $\delta_{j,k} = 1$ if $j = k$, otherwise 0. The result is produced from orthonormaility of the eigenfunctions. The last assertion is confirmed from the above two properties.
