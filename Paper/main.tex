...
The focus of this paper is to introduce FLR in a scalar-on-function setting. We will be using the standard FLR framework, which relates functional predictors to a scalar response as follows:
	...
	\begin{equation}\label{score_approx}
		\begin{split}
			...
			& \approx \sum_{j = 1}^{J} \left[a_{i,j}\sum_{k = 1}^{K} b_{k}^m \int_{0}^{1} \phi_j(s) \psi_{k}(s)\mathrm{d}s \right] =: \tilde{\xi}^{m,L}_{i}
		\end{split}
	\end{equation}
	...
	\begin{equation}\label{basis_exp_transf}
		\begin{split}
			...
			& = \alpha + \sum_{j = 1}^J b_j \left[\sum_{k = 1}^K d_k(\omega) \int_{0}^{1} \phi_j(s) \psi_k(s) \mathrm{d}s\right] + \sum_{j = 1}^{J} b_j  \int_{0}^{1} \phi_j(s) \delta_{X}^{K}(\omega)(s) \mathrm{d}s
			 + \sum_{k = 1}^{K} a_k  \int_{0}^{1} \delta_{\beta}^{J}(s)\phi_j(s) \mathrm{d}s + \epsilon(\omega)
		\end{split}
	\end{equation}
