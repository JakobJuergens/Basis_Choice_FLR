...
\begin{document}
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	{\color{orange}$$\{\sqrt{2}\sin(2\pi nx) | n \in \mathbb{N}\} \cup
...
There are several important aspects of functional regression in this functional setting that separate it from usual multiple regression according to \cite{kokoszka_introduction_2017}: In functional linear regression, the aim is not only to obtain an estimate of the function $\beta(s)$ — this estimate also needs to have a useful interpretation. Without it, there might be prediction, but the increase in understanding of the underlying question will be minimal. One aspect of a useful interpretation is that the estimate $\beta(s)$ should not jump in a seemingly random fashion, because an interpretation of this erratic behavior will often be impossible.
   ...
   If we approximate the scalar-on-functional regression by assuming a set of discrete observation points for all realizations of the data generating process as
	...
	For the simulation study, we deviate from the standard simulation setting. Instead of generating data by ourselves, we imposed the gasoline data which consists of 60 samples of Near Infrared (NIR) spectroscopy measured by 2-nm from 900 to 1,700 nm, and a response variable, octane rate. The dataset, which contains 60 i.i.d. observations with 401 measurements, allows to analyze samples of gasoline much faster and with the same reproducibility as standard tests (cf. \cite{Bohacs_Ovadi_Salgo1998}). The optimal number L of basis functions were chosen for each model through 10-fold Cross-Validation. Additionally, even for FPCR, we discovered what number of components produces the best. We finally progressed the application with the derived outcomes from our simulation study.
	
	\subsection{Generating Similar Curves}
	To avoid small sample problems, we generated 200 similar curves from the gasoline dataset. This is motivated by \hyperlink{KL}{Karhunen-Lo\'{e}ve Expansion}. First, the initial curves are expressed in terms of a generated bspline basis, created using 50 knots. These smooth curves are then centered({\color{orange} in this case, shouldn't mu be removed on the equation?}), before performing \hyperlink{KL}{Karhunen-Lo\'{e}ve Expansion}. It is supposed that each estimated score follows normal distribution with the fact that the mean is 0 and the variance equals its eigenvalue. Finally, we obtain the generated curves
	...
	The simulation study follows \cite{Reiss_2007b} as a guideline. Two different true coefficient functions, smooth function $f_1(t)$ and bumpy function $f_2(t)$, were chosen to differentiate their smoothness. The bumpy function was generated by refering to \cite{cardot_bumpyfunction_2002}. The two functions are described as: {\color{orange} we might define the interval of t on the functions}
    ...
    \end{equation}
		...
		The intention of comparisons based on the two functions was to represent which type of expansions turn out better simulation results in each case where the functions are bumpy and smooth. Two different error-terms $\epsilon$ were created by first generating $i.i.d.$ standard normal error and then multiplying them by a error variation $\sigma_e $ which is calculated such that the squared multiple correlation coefficient $R^2 = var(Xf) / (var(Xf) + \sigma^2_{e})$ is equal to 0.9 and 0.6. The two error-terms are then used to generate two sets of responses for $f \in \{f_1(t), f_2(t)\}$.
		...
		where $Z \sim \mathcal{N}(0,1)$ to test the methods with different signal-to-noise ratios. In total, we created four combinations for the simulations, using the two true coefficient functions and the two sets of responses. These four combinations are then used with different number of cubic bspline basis-function $(5,6,...,25)$ and fourier functions $(1,2,...,25)$ to predict the generated responses using the basis expansion approach and the FPCR approach. For the evaluation, we used 10f cross-validation. The simulation was done with R (version...). We carried out 2500 repetitions for each set of simulations. {\color{green} Information about version and link to repo / package in footnote??}
		
	\subsection{Put in appendix - Simulation setup cont.}
