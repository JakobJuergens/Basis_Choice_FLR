...
bspline_fpcr_appl_function <- function(rep, fold_size, nharm, seed, debug = FALSE){
  ...
  for (i in 1:rep) {
    ...
    # loop over different number of basis functions
    for (j in 1:length(n_basis)) {
      if (debug) {
        print(paste0("Basis specification: ", j, " out of ", length(n_basis)))
      }
      ...
      tryCatch(
        {
          for (m in 1:n_folds) {
            ...
            scores_mat  <- matrix(0, nrow = fold_size, ncol = nharm)
            ...
            dataframe <- as_tibble(cbind(responses[-sampling], train_fd$scores))
            
            # give correct names
            names(dataframe) <- c('responses', paste0('harm_', 1:nharm))
            
            # estimate linear models
            model <- lm(responses ~., dataframe) 
            
            # do prediction using estimated scores
            test_prediction <- unname(predict(object = model, newdata = scores_tib))
            
            # calculate errors
            error <- responses[sampling] - test_prediction
        ...
        error = function(cond) {
          # in Case an error occurs issue a warning with the corresponding message
          warning(paste0(
            "Problem occured in run ", i, " for ", n_basis[j], " b-spline basis functions for FPCA Run.",
            " Error message: ", cond
          ))
        }
      )
    ...
    for (j in 1:length(n_basis)) {
      ...
      tryCatch(
        ...
        error = function(cond) {
          # in Case an error occurs issue a warning with the corresponding message
          warning(paste0(
            "Problem occured in run ", i, " for ", n_basis[j], " monomial basis functions for FPCA Run.",
            " Error message: ", cond
          ))
        }
      )
