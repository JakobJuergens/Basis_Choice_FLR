        ...
        tryCatch(
          {
            ...
            MSE1_1 <- sum((model1_1$residuals)^2)/n_obs
            MSE1_2 <- sum((model1_2$residuals)^2)/n_obs
            MSE2_1 <- sum((model2_1$residuals)^2)/n_obs
            MSE2_2 <- sum((model2_2$residuals)^2)/n_obs
...
smallbasis <- create.bspline.basis(rangeval = c(0, 401), nbasis = 5, norder = 4)
...
dataframe1_1 <- as.data.frame(rbind(cbind(df1_1_train[,1], train_fd$scores),cbind(df1_1_test[,1], scores_mat)))
