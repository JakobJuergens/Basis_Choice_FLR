...
{
  # This is a modified function for dropind
  # Originated from https://github.com/cran/fda/blob/master/R/basisfd.R
  ...
  if (length(dropind) > 0) {
    ...
    for (i in 1:ndrop) {
      if (dropind[i] < 1 || dropind[i] > nbasis + 1)
...
{
  #This is a modified version for dropind
  #Originated from https://github.com/cran/fda/blob/master/R/create.fourier.basis.R
...
aa <- create.fourier.basis(rangeval = c(0, length(grid)), nbasis = 6, dropind = 7)
a <- create.fourier.basis(rangeval = c(0, length(grid)), nbasis = 6, dropind = 6)
b <- create.fourier.basis(rangeval = c(0, length(grid)), nbasis = 6, dropind = 4)

colnames(eval.basis(evalarg = seq(0,10,0.2) ,basisobj = aa))
colnames(eval.basis(evalarg = seq(0,10,0.2) ,basisobj = a))
colnames(eval.basis(evalarg = seq(0,10,0.2) ,basisobj = b))
