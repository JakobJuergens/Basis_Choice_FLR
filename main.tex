...
\begin{document}
	...
	Given a realization of a random function realizing in $\mathbb{L}^2$, it is possible to represent this realization in terms of its generating stochastic process. To motivate it, it is necessary to define the covariance function of a random function realizing in $\mathbb{L}^2$. Therefore, let $F: \Omega \mapsto \mathbb{L}^2[0,1]$ be such a random function.
	...
	With these it is possible to derive a set of sample analogs $\{(\hat{\lambda}_i, \hat{\nu}_i) \: \vert \: i \in \mathcal{I}\}$ for $\{(\lambda_i, \nu_i) \: \vert \: i \in \mathcal{I}\}$ as the solutions of the following equation. {\color{red} I think the number of principal components has to be smaller then the number of observations. So I can give more information about $\mathcal{I}$.}
	...
	In the simple scalar setting one of the most important tools in econometrics is the linear regression. Its goal is to predict the value of a dependent variable given a set of associated variables. This approach has two main purposes: inference and prediction. 
	...
	A general data generating process in this scenario could look like the following equation.
	
	\begin{equation}
		y = \alpha + \Psi(f) + \epsilon
	\end{equation}
	...
	However, the typical setup is often as follows mimicking the structure of the multivariate linear model extended from summation to integration.
	
	\begin{equation}
		y = \alpha + \int_{0}^{1} \beta(s)f(s) \mathrm{d}s + \epsilon
	\end{equation}
	
	Where $f(s)$ is the realization of a random function in $\mathbb{L}^2[0,1]$ and $\beta(s)$ is a coefficient function. 
